...
// 3D Football Game - CW2 - Lind Zariqi - w1712905
...
#ifdef WIN32
#include "gltools.h"		// Freeglut library installed with vcpkg in Windows PowerShell terminal: https://vcpkg.io/en/getting-started.html
...
const Vector3 vertBarPos = { 900, 600, 0 };
const Vector3 horzBarPos = { 1000, 600, 0 };
...
void* font = GLUT_BITMAP_HELVETICA_18; // chosen text from https://www.opengl.org/resources/libraries/glut/spec3/node76.html
...
void DisplayText(GLfloat x, GLfloat y, std::string message, int num) // text showing score and attempts left
{
	...
	glPushMatrix();
	if (num < 0) { glColor3f(0.0, 0.0, 0.0); } // Red
...
void checkWin() // Checks if all targets are hit, if true, resets the game
{
	bool win = true;
	for(Target* target: targets)
	{
...
{	
	...
	if (moveCamera) // If boolean is true, it follows the ball, otherwise it sits at a predetermined position
	{
		// view the scene
	...
	DisplayText(100, Wheight-40, "SCORE: ", score);
	DisplayText(100, Wheight - 20, "ATTEMPTS LEFT: ", attempts);
...
void specialKeyboard(int key, int x, int y) // keyboard controls
{
	if (!isShooting) // stops player from shooting after the ball has been shot
	{
		switch (key)
		{
...
void keyboard(unsigned char key, int x, int y) // keyboard controls
{
	switch (key)
	{
	case ' ': // space/z shoots
